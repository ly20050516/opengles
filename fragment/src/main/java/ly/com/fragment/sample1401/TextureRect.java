package ly.com.fragment.sample1401;
...
public class TextureRect {
    ...
    public TextureRect(Fragment1401SurfaceView mv) {
        //初始化顶点坐标与着色数据
        initVertexData();
        //初始化shader
        initShader(mv, 0, "vertex_sample_13_01/vertex_tex_x.glsl");
        initShader(mv, 1, "vertex_sample_13_01/vertex_tex_xie.glsl");
        initShader(mv, 2, "vertex_sample_13_01/vertex_tex_xy.glsl");
        //启动一个线程定时换帧
        new Thread() {
            public void run() {
    ...
    //初始化shader
    public void initShader(Fragment1401SurfaceView mv, int index, String vertexName) {
        //加载顶点着色器的脚本内容
        String mVertexShader = ShaderUtil.loadFromAssetsFile(vertexName, mv.getResources());
        //加载片元着色器的脚本内容
        String mFragmentShader = ShaderUtil.loadFromAssetsFile("vertex_sample_13_01/frag_tex.glsl", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
