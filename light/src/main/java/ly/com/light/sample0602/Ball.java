package ly.com.light.sample0602;
...
public class Ball {
    ...
    float r = 0.8f;
    public Ball(Light0602SurfaceView mv) {
        // 初始化顶点坐标与着色数据
        initVertexData();
        // 初始化shader
        initShader(mv);
    ...
    // 初始化shader
    public void initShader(Light0602SurfaceView mv) {
        // 加载顶点着色器的脚本内容
        mVertexShader = ShaderUtil.loadFromAssetsFile("light_sample_6_2/vertex.glsl",
                mv.getResources());
        // 加载片元着色器的脚本内容
        mFragmentShader = ShaderUtil.loadFromAssetsFile("light_sample_6_2/frag.glsl",
                mv.getResources());
        // 基于顶点着色器与片元着色器创建程序
