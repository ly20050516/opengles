package ly.com.light.sample0605;
...
public class Ball {
    ...
    float r = 0.8f;
    public Ball(Light0605SurfaceView mv) {
        // 初始化顶点坐标与着色数据
        initVertexData();
        // 初始化shader
        initShader(mv);
    ...
    // 初始化shader
    public void initShader(Light0605SurfaceView mv) {
        // 加载顶点着色器的脚本内容
        mVertexShader = ShaderUtil.loadFromAssetsFile("light_sample_6_5/vertex.glsl",
                mv.getResources());
        // 加载片元着色器的脚本内容
        mFragmentShader = ShaderUtil.loadFromAssetsFile("light_sample_6_5/frag.glsl",
                mv.getResources());
        // 基于顶点着色器与片元着色器创建程序
