...
public class Ball {
    ...
    float r = 0.8f;
    public Ball(Light0609SurfaceView mv) {
        // 初始化顶点坐标与着色数据
        initVertexData();
        // 初始化shader
        initShader(mv);
    ...
    // 初始化shader
    public void initShader(Light0609SurfaceView mv) {
        // 加载顶点着色器的脚本内容
        mVertexShader = ShaderUtil.loadFromAssetsFile("light_sample_6_9/vertex.glsl",
                mv.getResources());
        // 加载片元着色器的脚本内容
        mFragmentShader = ShaderUtil.loadFromAssetsFile("light_sample_6_9/frag.glsl",
                mv.getResources());
        // 基于顶点着色器与片元着色器创建程序
