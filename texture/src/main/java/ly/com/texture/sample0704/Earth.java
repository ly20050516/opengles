...
public class Earth {
    ...
    int vCount=0;
    public Earth(Texture0704SurfaceView mv,float r){
        //初始化顶点坐标与着色数据
        initVertexData(r);
        //初始化着色器
        initShader(mv);
    ...
    public void initShader(Texture0704SurfaceView mv){ //初始化着色器
        //加载顶点着色器的脚本内容
        mVertexShader= ShaderUtil.loadFromAssetsFile("texture_sample_7_4/vertex_earth.glsl", mv.getResources());
        ShaderUtil.checkGlError("==ss==");
        //加载片元着色器的脚本内容
        mFragmentShader=ShaderUtil.loadFromAssetsFile("texture_sample_7_4/frag_earth.glsl", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
        ShaderUtil.checkGlError("==ss==");
