...
public class Cuboid {
    ...
    public Cuboid(Vertex1302SurfaceView mv) {
        //初始化顶点坐标与着色数据
        initVertexData();
        //初始化shader
        intShader(mv);
    ...
    //初始化shader
    public void intShader(Vertex1302SurfaceView mv) {
        //加载顶点着色器的脚本内容
        mVertexShader = ShaderUtil.loadFromAssetsFile("vertex_sample_13_02/vertex_tex.glsl", mv.getResources());
        //加载片元着色器的脚本内容
        mFragmentShader = ShaderUtil.loadFromAssetsFile("vertex_sample_13_02/frag_tex.glsl", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
